...
//        picker.show();
    ...
    private void toWaitRootStatus(boolean flag) {
        ...
        if (mDetailInfo != null) {
            ...
            if (mDetailInfo.getOrderDes().getStatus() == 2 && !flag) {
                ...
                mHourLastTv.setTextColor(Color.parseColor("#ffffff"));
            } else {
                mHourLastTv.setVisibility(View.GONE);
            }
        }
    ...
    private OrderDes getOrderDes(JSONObject detailJson) {
        ...
        try {
            ...
            if (detailJson.has("settlementStatus")) {
                settleStatus = detailJson.getInt("settlementStatus");
            }
...
//                }
    ...
    private void startSign() {
        ...
        BillApi billApi = KHttpWorker.ins().createHttpService(LoginApi.URL, BillApi.class);
        Map<String, String> params = new HashMap<>();
        params.put("token", ConfigManager.getInstance(OrderDetailActivityNew.this).getUserSession());
        params.put("userCode", ConfigManager.getInstance(OrderDetailActivityNew.this).getUserCode());
        params.put("orderId", orderId);
                ...
                if (code == 0) {
                    ...
                    mHandler.post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            Utils.showShortToast(OrderDetailActivityNew.this, msg + ": " + code);
                        }
                    });
                }
    ...
    private void saveCompleteInfoOnly() {
        ...
        if ("null".equals(maintainInfo.getFault()) || maintainInfo.getFault().length() == 0) {
            Utils.showShortToast(OrderDetailActivityNew.this, "请填写维修项完整信息");
            return;
        }
        ...
        request.enqueue(new retrofit2.Callback<JsonObject>() {
            ...
            public void onResponse(retrofit2.Call<JsonObject> call, retrofit2.Response<JsonObject> response) {
                ...
                if (code == 0) {
                    ...
                    Intent intent = new Intent(OrderDetailActivityNew.this, OrderPayQRCodeActivity.class);
...
//        Utils.finishActivity(this);
    ...
    void onClickCopy() {
        ...
        if (TextUtils.isEmpty(mNumTv.getText().toString())) {
            return;
        }

        try {
            ClipboardManager cm = (ClipboardManager) getSystemService(Context.CLIPBOARD_SERVICE);
            cm.setText(mNumTv.getText());
