...
public class UpdateLogicImpl implements IUpdateLogic {
	...
	public void checkNeedUpdate() {
		UpdateInfo updateInfo = APIFactory.getUpdateAPI(mContext).getUpdateInfo();
		if(updateInfo == null) return;
		
		String remoteVersion = updateInfo.getVersion();
		String localVersion = getLocalVersion();
		
		if (remoteVersion.compareTo(localVersion) > 0) {
			ConfigManager.getInstance(mContext).setNeedUpdate(true);
			ConfigManager.getInstance(mContext).setUpdateURL(updateInfo.getUrl());
			ConfigManager.getInstance(mContext).setUpdateVersion(updateInfo.getVersion());
			ConfigManager.getInstance(mContext).setUpdateDescription(updateInfo.getDescription());
		}
		else {
			ConfigManager.getInstance(mContext).setNeedUpdate(false);
			ConfigManager.getInstance(mContext).setUpdateURL("");
			ConfigManager.getInstance(mContext).setUpdateVersion("");
			ConfigManager.getInstance(mContext).setUpdateDescription("");
		}
	}
